...
let s:Instance_prefix = " "
let s:Instance_suffix = "_Inst_0"
...
fun! <SID>Inst_Part_Format(module_num, module_name, para_list, port_list)
    ...
    while i < a:module_num
        ...
        if empty(a:para_list[i])    "has no parameter
			...
			let inst = inst.a:module_name[i] . s:Instance_prefix . toupper(a:module_name[i]) . s:Instance_suffix . "(\n"
        else                        "has parameters
            let has_para_flag = 1
        ...
        if has_para_flag == 1           "has parameter
            ...
            let inst = inst . s:Instance_prefix toupper(a:module_name[i]) . s:Instance_suffix . "(\n"
        endif
